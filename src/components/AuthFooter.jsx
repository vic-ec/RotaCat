...
// sheet (bg-accent-light); the default (on-fill/70) is for the desktop
// accent background, which is teal in the light theme and near-black in
// the dark one -- hence the token rather than a literal white. `topGap` lets a caller match this line's margin-top
...
export default function AuthFooter({ onLight = false, topGap = 'mt-3', compact = false }) {
  return (
    <p className={`${topGap} text-center ${compact ? 'text-[10px]' : 'text-xs'} ${onLight ? 'text-ink-muted' : 'text-on-fill/70'}`}>
