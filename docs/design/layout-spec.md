...
Lives inside the list's own "Select all" header row (`SelectAllRow`), appearing the moment ≥1 row is checked. Same position and styling everywhere "select all" exists — including mobile, where it stays in the header rather than becoming a separate bottom-fixed bar (see the implementation note below).

```
☑ SELECT ALL              3 SELECTED   [ Approve ]  [ Reject ]  Cancel
```

- Left: the select-all checkbox and label, always present.
...
- Below `md`, the actions render as the same circular icon buttons `ApprovalRow` uses (check / cross / ✕) so the count and all three actions still fit beside "Select all" on a narrow row; `md:` and up spells them out as text buttons.

**Implementation note:** this replaced an earlier separate `BulkActionBar` that sat above the list on desktop and fixed itself to the bottom of the viewport on mobile. Folding it into the select-all header keeps the selection state and the actions on that selection in one place on every viewport, and frees the mobile bottom-right corner for the FAB (§15), which previously had to hide itself whenever a row was checked.
...
- The stack reveals one button at a time: each scales 0→1 and fades in over 75ms on an overshoot curve, and the next starts only once the previous has finished. **Closing is not animated at all** — the stack is gone the frame the FAB is tapped, including the ✕→⊕ rotation. Dismissing is a correction, and there's nothing to read on the way out; a cascade played backwards only makes the user wait to get their screen back. The opening shape is borrowed from [nambicompany/expandable-fab](https://github.com/nambicompany/expandable-fab), the Android widget this pattern is modelled on, at a shorter per-button duration than its own 125ms default — a sequential cascade costs that per button, so the total is what has to stay tolerable. Respects `prefers-reduced-motion` (all appear at once). Collapsed buttons stay in the DOM so they can animate, so they're held inert (`aria-hidden`, out of the tab order, no pointer events) rather than merely invisible.
...
- A page with its own bottom-fixed element hides the FAB via its `hidden` prop rather than letting the two overlap. Bulk selection no longer needs this — its actions live in the select-all header (§8), not a bottom bar, so the FAB stays available while rows are checked.
...
- When items are selected, the bulk actions (§8) appear inside the list's "Select all" header as icon buttons. This supersedes the spec's original **sticky bar fixed to the bottom of the viewport**: that bar collided with the FAB (§15) for the same corner, and split the selection state from the actions on it.
...
- [x] Extract `PageHeader`, `Breadcrumb`, `Toolbar`, `SectionLabel`, `ListRow`, `Tag`, and a shared bulk action bar (later folded into `ListRow`'s `SelectAllRow`, §8) as shared components — plus `Modal` and `SlideOverPanel`, not originally listed but built the same pass (`src/components/`)
...
- [x] Make bulk action bar sticky-bottom on mobile — built as `BulkActionBar`, then **superseded**: the actions now live in `SelectAllRow` itself on every viewport (§8), so nothing is fixed to the bottom edge. Roster's per-group bulk-select header remains its own inline swap — Active has two independently-selectable lists (Drafts + Published) at once
