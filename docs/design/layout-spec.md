...
- The stack reveals one button at a time, ~45ms apart, fading and scaling up from the trigger — outward from the FAB on open, back toward it on close, so it visibly unwinds rather than replaying the opening order. Respects `prefers-reduced-motion` (all appear at once). Collapsed buttons stay in the DOM to animate, so they're held inert (`aria-hidden`, out of the tab order, no pointer events) rather than merely invisible.
