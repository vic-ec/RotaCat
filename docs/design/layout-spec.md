...
Lives in the "Select all" header row itself (`SelectAllRow`), filling its right-hand side the moment ≥1 row is checked. Same position and styling everywhere "select all" exists.

```
[x] SELECT ALL                 3 selected   [ Approve ]  [ Reject ]  Cancel
```

- Left: the select-all checkbox and label (always there), then a live count of selected items.
...
- Mobile keeps the identical row, with each action as an icon-only circular button (✓ / ✕) plus an ✕ Cancel, so the set still fits beside the label at phone widths. This replaced an earlier bottom-fixed bar — putting the actions where the checkbox already is beats splitting the selection UI across the top and bottom edges of the viewport, and it frees the bottom edge for the Toolbar FAB (§15).
...
- A page with its own bottom-fixed element hides the FAB via its `hidden` prop rather than letting the two overlap. No page currently needs this — bulk selection used to put a bar on the same bottom edge, and now keeps its actions in the `SelectAllRow` header instead (§8) — but the prop stays for the next page that does.
...
- When items are selected, the bulk actions (§8) stay in the "Select all" header row and switch to icon-only buttons. They were briefly a sticky bottom bar instead (thumb-reachable), but that collided with the Toolbar FAB and put the actions a screen away from the checkbox that triggered them.
...
- [x] Extract `PageHeader`, `Breadcrumb`, `Toolbar`, `SectionLabel`, `ListRow`, `Tag`, bulk actions (originally a standalone `BulkActionBar`, since folded into `ListRow`'s `SelectAllRow`) as shared components — plus `Modal` and `SlideOverPanel`, not originally listed but built the same pass (`src/components/`)
...
- [x] Make bulk action bar sticky-bottom on mobile — built as `BulkActionBar`, then **reverted**: the actions now live inline in `SelectAllRow` on every viewport (§8), icon-only below `md`. A bar fixed to the bottom edge fought the Toolbar FAB for the same corner and sat far from the checkbox that summoned it. Roster's per-group bulk-select header stays its own inline swap either way — Active has two independently-selectable lists (Drafts + Published) at once
