...
// doctors from the same capacity column (MO / Registrar / EC Intern /
// OT Intern) can be on leave at once, and — combined across MO,
// Registrar, and EC Intern only — a shared cap of 2 "full-time EC
// doctors" at once (e.g. 1 MO + 1 Registrar is fine, but 1 MO + 1 Registrar
// + 1 EC Intern is not, even though each individual column is still
// within its own limit). OT Intern is a separate pool, capped
...
async function checkAnnualLeaveCapacity({ profileId, dateFrom, dateTo }) {
  ...
  if (LEAVE_FULL_TIME_GROUP_KEYS.includes(columnKey)) {
    ...
    if (fullTimeBreach) {
      throw new Error(`No more than ${maxTotal} full-time EC doctors (MO/Registrar/EC Intern combined) may be on leave at once, and that's already reached on ${fullTimeDates[0]}. Adjust the dates and try again.`)
    }
  }
}
