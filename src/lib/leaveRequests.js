...
// Registrar, and EC COSMO/Intern together — a shared cap of 2 "EC full-time
...
// within its own limit). OT COSMO/Intern sits outside that group — its own
// per-column cap (1) is the only check that applies to it. Checked against
// every other pending or approved annual-leave request (rejected/withdrawn
// never count, same as the double-booking check above). No-op for any other
// leave type, or for a category with no capacity column (Other).
async function checkAnnualLeaveCapacity({ profileId, dateFrom, dateTo }) {
  ...
  if (LEAVE_FULL_TIME_GROUP_KEYS.includes(columnKey)) {
    ...
    if (fullTimeBreach) {
      throw new Error(`No more than ${maxTotal} EC full-time doctors (MO/Registrar/EC COSMO/Intern combined) may be on leave at once, and that's already reached on ${fullTimeDates[0]}. Adjust the dates and try again.`)
    }
  }
}
