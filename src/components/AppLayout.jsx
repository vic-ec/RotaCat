    ...
    <div className="flex min-h-screen bg-accent-light">
      ...
      <aside className="sticky top-0 hidden h-screen w-60 flex-col border-r border-accent/50 bg-canvas-raised md:flex">
        <div className="px-5 py-6">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            ...
            {profile && (
              <span className="text-xs text-ink-muted">Logged in as {profile.name} {profile.surname}</span>
            )}
          </div>
          {profile && (
        ...
        <div className="border-t border-accent/25 p-3">
          <button
            ...
            title="Sign out"
            aria-label="Sign out"
            className="group flex items-center gap-3 overflow-hidden rounded px-3 py-2.5 text-sm font-medium text-ink-light transition-colors hover:bg-accent-light hover:text-ink-light active:bg-accent active:text-canvas-raised"
          >
      ...
      <header className="fixed inset-x-0 top-0 z-10 flex items-center justify-between gap-2 border-b border-accent/50 bg-canvas-raised px-4 py-3 md:hidden">
        <div className="flex min-w-0 flex-wrap items-baseline gap-x-2">
          ...
          {profile && (
            <span className="truncate text-[11px] text-ink-muted">Logged in as {profile.name} {profile.surname}</span>
          )}
        </div>
        <button
          onClick={handleSignOut}
